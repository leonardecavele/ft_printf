...
/*   Updated: 2025/11/06 17:31:14 by ldecavel         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
static int	szputnbr_en(t_64 n, int en)
{
	int		sz;
	char	c;
...
static int	szputhex_en(t_u64 n, char a, char c, int en)
{
	int		sz;
	char	b[16];
	int		d;
...
int	szputpm_en(int en, va_list pm, char c, int n)
{
	...
	if (c == 'c')
		...
		sz += szputhex_en((t_u64)va_arg(tmp, void *), 'a', c, en);
	else if (c == 'd' || c == 'i')
		sz += szputnbr_en((int)va_arg(tmp, int), en);
	else if (c == 'u')
		sz += szputnbr_en((unsigned int)va_arg(tmp, unsigned int), en);
	else if (c == 'x')
		sz += szputhex_en((t_u64)va_arg(tmp, unsigned int), 'a', c, en);
	else if (c == 'X')
		sz += szputhex_en((t_u64)va_arg(tmp, unsigned int), 'A', c, en);
	else if (c == '%')
		sz += szputchar_en('%', en);
