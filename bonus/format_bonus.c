...
/*   Updated: 2025/11/10 16:33:31 by ldecavel         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
static int	prefix(char c, t_format *f)
{
	...
	n = 0;
	if (ft_strchr("di", c) && f->val < 0)
		n += write(1, "-", 1);
	else if (ft_strchr("di", c) && f->flags & FPL)
		n += write(1, "+", 1);
	else if (ft_strchr("di", c) && f->flags & FSP)
		n += write(1, " ", 1);
	if (((c == 'x' && f->flags & FHS) || c == 'p') && f->val)
		n += write(1, "0x", 2);
	else if (c == 'X' && f->flags & FHS && f->val)
		n += write(1, "0X", 2);
	return (n);
}
...
static int	lpad(char c, t_format *f)
{
	...
	n = 0;
	if (f->pre > -1 && f->pre < 6 && c == 's' && f->val == 0)
	{
		while (f->wid--)
			n += write(1, " ", 1);
		f->pad = 0;
...
int	format(char c, va_list pm, t_format *f)
{
	...
	if (f->val == 0 && f->pre == 0 && ft_strchr("diuxX", c))
		f->n = 0;
	if (c == 's' && f->pre > -1)
		f->n = min(f->n, f->pre);
	pre = 0;
	if (ft_strchr("di", c) && (f->flags & FPL || f->flags & FSP || f->val < 0))
		pre = 1;
	if ((ft_strchr("xX", c) && (f->flags & FHS) && f->val) || (c == 'p' && f->val))
		pre = 2;
	...
	n = 0;
	n += lpad(c, f);
	if (c == 's' && f->pre > -1 && f->pre < 6 && f->val == 0)
		return (n);
	n += prefix(c, f);
	n += zer(f);
	if (!(ft_strchr("diuxX", c) && f->pre == 0 && f->val == 0))
